/* Change these items for each quiz
 * Other than the items in this file, the only things that will need to be changed for each quiz are the images in the Images folder and the reference to this document in the HTML file.
 * The part to change in the HTML file is near the bottom. 
 * It looks like this: <script src="quiz_#.js"></script> 
 * Just change the # to the number of the quiz. */

// This text is what is displayed on the tab of the web page.
var tabTitle = "Map 30: The Cold War";

// This text is what is displayed at the top of the web page
var quizHeader = "Geography Quiz 30: Cold War Alliances";

// This is used to change the size of the div that has the map images. The first value is the map's width; the second is its height.
var imgDimension = [1549, 786];

// This is used to change the areas used in the image map "imgClick"
// For each item, "x1, y1, x2, y2" is used to create the rect areas of the image map.
var mapDimension = [
  /* A */ " 773,  148,  805,  184",
  /* B */ " 993,  101, 1021,  138",
  /* C */ "1327,  497, 1357,  536",
  /* D */ " 475,  245,  509,  279",
  /* E */ " 886,  127,  912,  163",
  /* F */ " 373,  184,  394,  217",
  /* G */ " 987,  216, 1019,  253",
  /* H */ "1253,  158, 1287,  205",
  /* I */ " 397,   96,  421,  138",
  /* J */ " 753,   48,  777,   95"];

// This text is displayed next to "Selected Question: " when the associated question is selected. The text comes from the original quiz.
var qTextArray = [
  /* 1. */  "Australia",
  /* 2. */  "Great Britain",
  /* 3. */  "United States",
  /* 4. */  "Canada",
  /* 5. */  "Soviet Union",
  /* 6. */  "Iran, a member of the Central Treaty Organization and an important source of oil",
  /* 7. */  "South Korea, where the United States fought a war",
  /* 8. */  "Cuba, a Soviet ally in the West",
  /* 9. */  "Turkey, a United States ally that borders the Soviet Union",
  /* 10. */ "France, a member of the North Atlantic Treaty Organization (NATO)"];

// At this time, this array is not used for anything other than supplying the value for qArray[i].answer. No function currently exists to grade the quiz as to avoid giving the answers to the student.
// Enter the letter representing the correct location for each question.
var qAnsArray = [
  /*  1. */  "C",
  /*  2. */  "J",
  /*  3. */  "F",
  /*  4. */  "I",
  /*  5. */  "B",
  /*  6. */  "G",
  /*  7. */  "H",
  /*  8. */  "D",
  /*  9. */  "E",
  /*  10. */ "A"
]

// This text is displayed next to "Selected Location: " when the associated location is selected. The intention is to assist the student in finding the location on the map.
var lTextArray = [
  /* A. */ "A is a region directly north of the map's center. It is south of region J.",
  /* B. */ "B is a very large region that is in the northeast portion of the map.",
  /* C. */ "C is a large region in the southeast portion of the map.",
  /* D. */ "D is a small region in the northwest portion of the map. It is southeast of region F.",
  /* E. */ "E is a region in the northeast portion of the map. It is southwest of region B and northwest of region G.",
  /* F. */ "F is a large region in the northwest portion of the map. It is northwest of region D and south of region I.",
  /* G. */ "G is a region in the northeast portion of the map. It is south of region B and southeast of region E.",
  /* H. */ "H is a very small region in the northeast portion of the map. It is southeast of region B.",
  /* I. */ "I is a large region in the northwest portion of the map. It is north of region F.",
  /* J. */ "J is a small region directly north of the map's center. It is north of region A."];
